...
// validity switch, and passes its Store.Add on to the service worker (install.ts).
...
import { enabledCertificates, extendedValidity } from "./roots.ts";
...
window.addEventListener("message", (event: MessageEvent) => {
  ...
  if (data.type === ROOTS_REQUEST) {
    void Promise.all([enabledCertificates(), extendedValidity()]).then(([{ roots, intermediates }, extended]) => {
      window.postMessage({ type: ROOTS_RESPONSE, id, certificates: roots, intermediates, extendedValidity: extended } satisfies RootsResponse, "*");
    });
