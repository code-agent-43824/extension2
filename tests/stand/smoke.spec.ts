...
test("Rutoken Plugin finds the fake token and signs with its certificate", async () => {
  ...
  const result = await page.evaluate(
    async ({ loadPlugin, pin, caPem }) => {
      ...
      return { verifyWithoutCa, version: await plugin.version, devices, certCount: certs.length, subject: JSON.stringify(parsed.subject), verifyAttached, verifyDetached };
    },
