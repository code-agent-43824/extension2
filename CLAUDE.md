...
- `src/extension/` — the extension's own side, with Chrome types (`src/extension/tsconfig.json`): `sites.ts` (the
  ...
  (service worker keeping the registration in sync), `popup.*` (the button's window), `options.*` (site list and root store), `roots.ts` (the root certificate store in
  `chrome.storage.local`) + `roots-view.ts` (its cards on the options page) + `roots-bridge.ts` (isolated-world
  content script registered beside page.js, handing it the enabled roots over postMessage) +
