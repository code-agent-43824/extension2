...
- `src/extension/` — the extension's own side, with Chrome types (`src/extension/tsconfig.json`): `sites.ts` (the
  ...
  (service worker keeping the registration in sync and running `install.ts`), `popup.*` (the button's window), `options.*` (site list, the extended validity switch and certificate stores), `roots.ts` (the two certificate stores in
  ...
  in the service worker: a root only after the user's yes in `confirm.*`, the extension's own window) +
...
- `tests/stand/` — Playwright tests on the stand; `harness.ts` launches Chromium with the adapter and serves pages, offline;
  ...
  on the options page; `validity.spec.ts` the extended validity check and the demo page adding its root; `with-cryptopro-csp.spec.ts` (opt-in)
