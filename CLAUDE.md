...
- `src/extension/` — the extension's own side, with Chrome types (`src/extension/tsconfig.json`): `sites.ts` (the
  ...
  (service worker keeping the registration in sync), `popup.*` (the button's window), `options.*` (site list and certificate stores), `roots.ts` (the two certificate stores in
  ...
  (isolated-world content script registered beside page.js, handing it the enabled roots and intermediates over postMessage) +
...
- `src/page/` — the MAIN-world content script, bundled into one `page.js`: `main.ts` (entry), `cadesplugin.ts`
  ...
  `pin-dialog.ts` (the PIN window, in a shadow root), `roots.ts` (asks the bridge for the "Root" and "CA" stores), `objects/enrollment.ts` (X509Enrollment for CA pages:
